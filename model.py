...
class EncoderNet(nn.Module):
    def __init__(self, width, height, device, latent_size, full_connected_size=320, input_channels=4,
                 kernel_size=3, encoder_stride=2, decoder_stride=1, extra_layer=False):
        super().__init__()
        #nn.Conv2d(in_channels=input_channels, out_channels=, kernel_size=kernel_size)
...
class Monet(nn.Module):
    def __init__(self, conf, height, width, device, latent_size):
        super().__init__()
        self.conf = conf
